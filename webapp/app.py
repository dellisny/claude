...
# ===========================================================================
# HPL-P1 — Unusual Options Screener
# ===========================================================================

import sqlite3 as _sqlite3
import xml.etree.ElementTree as _ET

_HPL_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "hpl_scans.db")
_HPL_SCAN_STATE: dict = {}
_HPL_CACHE: dict = {}
_HPL_CACHE_TTL = 3600
...
def _hpl_conn():
    c = _sqlite3.connect(_HPL_DB)
    c.row_factory = _sqlite3.Row
    return c

def _hpl_init():
    os.makedirs(os.path.dirname(_HPL_DB), exist_ok=True)
    with _hpl_conn() as c:
...
def _hpl_get_tickers() -> list:
    try:
        df = pd.read_html("https://en.wikipedia.org/wiki/List_of_S%26P_500_companies")[0]
        return df['Symbol'].str.replace('.', '-', regex=False).tolist()
    except Exception:
        return _HPL_FALLBACK_TICKERS

# --- Options screener ---

_HPL_MIN_VOL_OI = 2.0
_HPL_MIN_PREMIUM = 250_000
_HPL_MIN_DTE = 7
_HPL_MAX_DTE = 21

def _hpl_scan_ticker(symbol: str) -> list:
    signals = []
    try:
        t = yf.Ticker(symbol)
        today = datetime.now().date()
        is_etf = symbol in _HPL_INDEX_ETFS
        near_earnings = False
        try:
        ...
        for expiry in t.options:
            ...
            dte = (exp - today).days
            if not (_HPL_MIN_DTE <= dte <= _HPL_MAX_DTE):
                continue
            try:
...
_HPL_EDGAR_HEADERS = {"User-Agent": "HPL-P1 Options Screener dellis@pobox.com"}

def _hpl_load_cik_map() -> dict:
    cached = _HPL_CACHE.get('cik_map')
    if cached and (time.time() - cached['ts']) < 86400:
        return cached['data']
    try:
        r = requests.get("https://www.sec.gov/files/company_tickers.json",
                         headers=_HPL_EDGAR_HEADERS, timeout=15)
        data = {v['ticker'].upper(): str(v['cik_str']) for v in r.json().values()}
        _HPL_CACHE['cik_map'] = {'data': data, 'ts': time.time()}
        return data
    except Exception:
        return {}

def _hpl_parse_form4_xml(cik_int: int, accession_nodash: str, primary_doc: str, filing_date: str) -> list:
    url = f"https://www.sec.gov/Archives/edgar/data/{cik_int}/{accession_nodash}/{primary_doc}"
    try:
        r = requests.get(url, headers=_HPL_EDGAR_HEADERS, timeout=10)
...
def _hpl_get_insider_trades(ticker: str, days: int = 30) -> list:
    cik_map = _hpl_load_cik_map()
    ...
    try:
        ...
        r = requests.get(f"https://data.sec.gov/submissions/CIK{cik_padded}.json",
                         headers=_HPL_EDGAR_HEADERS, timeout=10)
        if r.status_code != 200:
            return []
        data = r.json()
        ...
        for i, ft in enumerate(form_types):
            ...
            if i < len(primary_docs) and primary_docs[i]:
                ...
                trades.extend(_hpl_parse_form4_xml(int(cik), accession_nodash, primary_docs[i], dates[i]))
            if len(trades) >= 10:
                break
        return trades[:10]
    except Exception:
...
def _hpl_load_congress_trades() -> list:
    cached = _HPL_CACHE.get('congress_trades')
    if cached and (time.time() - cached['ts']) < _HPL_CACHE_TTL:
        return cached['data']
    trades = []
    for url, chamber, name_key in [
        ...
        try:
            ...
            pass
    _HPL_CACHE['congress_trades'] = {'data': trades, 'ts': time.time()}
    return trades

def _hpl_get_congress_trades(ticker: str, days: int = 60) -> list:
    cutoff = (datetime.now() - timedelta(days=days)).date()
    result = []
    for t in _hpl_load_congress_trades():
...
def _hpl_score(signal: dict) -> float:
    score = 0.0
    flags = []
...
def _hpl_run_scan(scan_id: str):
    tickers = _hpl_get_tickers()
    total = len(tickers)
    _HPL_SCAN_STATE[scan_id] = {
        'status': 'scanning', 'total': total, 'processed': 0,
        'current_ticker': '', 'signals_found': 0,
    }
    with _hpl_conn() as c:
        c.execute(
            "UPDATE hpl_scans SET total_tickers=? WHERE id=?",
            (total, scan_id)
        )
    all_signals = []
    processed = 0
    try:
        with ThreadPoolExecutor(max_workers=8) as ex:
            futures = {ex.submit(_hpl_scan_ticker, t): t for t in tickers}
            for future in as_completed(futures):
                ...
                try:
                    ...
                    pass
                processed += 1
                _HPL_SCAN_STATE[scan_id].update({
                    'processed': processed,
        ...
        _HPL_SCAN_STATE[scan_id]['status'] = 'enriching'
        with _hpl_conn() as c:
        ...
        for i, ticker in enumerate(unique):
            insider = _hpl_get_insider_trades(ticker)
            congress = _hpl_get_congress_trades(ticker)
            for s in all_signals:
                if s['ticker'] == ticker:
                    s['insider_trades'] = insider
                    s['congress_trades'] = congress
            _HPL_SCAN_STATE[scan_id]['enrich_progress'] = f"{i+1}/{len(unique)}"
        for s in all_signals:
            _hpl_score(s)
        ...
        _HPL_SCAN_STATE[scan_id].update({'status': 'complete', 'signals_found': len(all_signals)})
    except Exception as e:
        with _hpl_conn() as c:
            c.execute("UPDATE hpl_scans SET status='error', error=? WHERE id=?", (str(e), scan_id))
        _HPL_SCAN_STATE[scan_id].update({'status': 'error', 'error': str(e)})

# --- Routes ---

@app.get("/hpl-p1", response_class=HTMLResponse)
async def hpl_page(request: Request):
    with _hpl_conn() as c:
    ...
    return templates.TemplateResponse("hpl-p1.html", {
        "request": request, "active": "hpl-p1", "scans": scans,
    })

@app.post("/hpl-p1/scan")
async def hpl_start_scan():
    ...
    _threading.Thread(target=_hpl_run_scan, args=(scan_id,), daemon=True).start()
    return JSONResponse({"scan_id": scan_id})

@app.get("/hpl-p1/scan/{scan_id}/progress")
async def hpl_scan_progress(scan_id: str):
    async def generate():
        while True:
            with _hpl_conn() as c:
...
@app.get("/hpl-p1/scan/{scan_id}/results")
async def hpl_scan_results(scan_id: str):
    with _hpl_conn() as c:
